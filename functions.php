...
function fire_css(){
    ...
    wp_enqueue_style( 'css-ie', get_template_directory_uri() . "/assets/css/ie.css", array( ) );
...
function button_customize_set($id,$i,$type,$links,$texts,$condition = 0,$extra = 0,$anchor=null)
{
...
?>
<a 
href="<?php echo ($condition == 2)? get_site_url()."/".get_the_title(get_post_meta( $id, $links.$id.$i, true )) : (get_post_meta( $id, $anchor.$id.$i, true ) == null ) ? get_post_permalink(get_post_meta( $id, $links.$id.$i, true )) : "#".get_post_meta( $id, $anchor.$id.$i, true ); ?>" 
class="fr-button-link-global" 
style=" background:<?php echo ($background == "no")? 'transparent' : $primary; ?> ;
        border-color:<?php echo $primaryborder; ?> ;
...
<span><?php echo get_post_meta( $id, $texts.$id.$i, true ); ?></span>
...
function prefix_send_email_to_admin() {
        ...
        if (isset($_POST["fr-front-nonce"]) || wp_verify_nonce($_POST["fr-front-nonce"], basename(__FILE__))):
            if(isset($_POST['submit'])) {
                ...
                if(isset($_POST['recaptcha']))
				    $post_data['data-sitekey']  = $_POST['recaptcha'];
				...
				$curl_connection = curl_init($o_action);
				//set options
            ...
            if(isset($_POST["fire-id"])):
                $id =  $_POST["fire-id"];
                $to_mail .= "From ".get_the_title($id)."<br>";
           ...
           //sending email
        //    $to = 'societyhater2@gmail.com'; //mail to be send to
        //    $subject = 'New Lead from'.get_the_title($id);
        //    $body = $to_mail;
 ...
 function prefix_send_email_to_admin2() {
    /**
     * At this point, $_GET/$_POST variable are available
     *
     * We can do our normal processing here
    ...
    $url = $_POST['_wp_http_referer'];
    if(isset($_POST["fr-front-floater-nonce"]) || wp_verify_nonce($_POST["fr-front-floater-nonce"], basename(__FILE__))):
                           //sending email

        if(isset($_POST['submit'])) {  
            $id = $_POST['fire-id']; 
            $i = $_POST['fire-counter']; 
            $field1 = sanitize_text_field($_POST['fire-first']);
            $field2 = sanitize_text_field($_POST['fire-second']);
            $to = get_post_meta( $id,'meta-box-floater-email-'.$id.$i, true ); //mail to be send to
            $subject = sanitize_text_field(get_post_meta( $id,'meta-box-floater-title-'.$id.$i, true ));
            $body = get_post_meta( $id,'meta-box-floater-placeholder1-'.$id.$i, true ).':'.$field1
                    .'<br>'.get_post_meta( $id,'meta-box-floater-placeholder2-'.$id.$i, true ).':'.$field2;
            $headers = array('Content-Type: text/html; charset=UTF-8');
            
            $results = wp_mail( $to, $subject, $body, $headers );
            var_dump($subject);
            die();
        }
        wp_safe_redirect(
 ...
 add_action( 'admin_post_nopriv_fire_floater_form', 'prefix_send_email_to_admin2');
 add_action( 'admin_post_fire_floater_form', 'prefix_send_email_to_admin2');
...
// function that runs when shortcode is called
 function FIRE_FORM_FRONT($atts) { 
  ...
  $old_action = $action; 
    ...
    $to_email = esc_html(get_post_meta($post_id, "meta-box-fr-forms-email-".$post_id, true));
    $res = ' ';
    if($design == 'dark'){
        $ied = "footer-form";
    ...
    $act = ($embed == 'mailchimp')? $old_action : esc_url(admin_url('admin-post.php'));
    $tar = ($embed == 'mailchimp')? 'target = "_blank"' : ' ';
    $res .= '<form id="'.$ied.'" action="'.$act.'" data-parsley-validate="" data-parsley-focus="none" method="POST" '.$tar.' >'.wp_nonce_field(basename(__FILE__), "fr-front-nonce").'';
    if($capt !== ' '){
        $res .= '
    ...
    $res .= '
	<input type=hidden name="fire_id" value="'.$post_id.'">
...
<input type=hidden name="to_email" value="'.$to_email.'">';
     if ($count > 0) {
        for($i = 0; $i < $count ; $i++):
            ...
            switch($type){
                case "hidden":
                    $res .= '<div class="row field-col d-none">
                    <input type="hidden" class="form-control '.$x_class.'" placeholder=" "  id = "'.$ids.'" name = "'.$name.'" value="'.get_post_meta($post_id, "meta-box-fr-forms-options".$i."-".$post_id, true).'" data-parsley-trigger="focusout">
                    </div>';
                break;
                case "textarea":
    ...
    $res .= '     
        ...
        <input type="hidden" name="fire-actions" value="'.$old_action.'">
        <input type="hidden" name="recaptcha" value="'.$siteKey.'">
    ...
    class="'.$btn_class.'" 
                ...
                name="submit" 
                value="Send">
    <span>'.get_post_meta( $post_id, "meta-box-fr-forms-btext-".$post_id, true ).'</span>
